    ...
    while(TRUE)
    {
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            for (i = 0; i < max_clients; i++)
            {
                ...
                if( client_socket[i] == 0 )
                {
                    ...
                    bufferTest[0] = i;
                    bufferTest[1] = '\0';
                    send(new_socket, bufferTest, 2,0);
                    printf("Sending client id: %d\n", i);
                    if (i == 1) {
                        bufferTest[0] = 'c';
                        bufferTest[1] = '\0';
                        send(client_socket[0],bufferTest, 2, 0 );
                        send(client_socket[1],bufferTest, 2, 0 );
                        printf("Send ready sign 'c' to both clients\n");
                    }
        ...
        for (i = 0; i < max_clients; i++)
        {
            ...
            if (FD_ISSET( sd , &readfds))
            {
                ...
                //incoming message
                if ((valread = recv( sd , tmp,((sizeof(char)*4)+(sizeof(float)*3)) ,0)) == 0)
                {
                ...
                else
                {
                    ...
                    if (i == 0) {
                        ...
                        bufferDirection[1] = '\0';*/
                        sd = client_socket[1];
                        send(sd , tmp , ((sizeof(char)*4)+(sizeof(float)*3)) , 0 );
                    } else if (i == 1 ){
                        /*char bufferDirection[2];
                        bufferDirection[0]= buffer[0];
                        bufferDirection[1] = '\0';*/ 
